...
class Player{
	...
	public:
		...
		void wear(vector<Equipment> &);
		void get();
		void walk(int ****,vector<Equipment> &);
		void beTrapped();
...
void Player::showss(){
	gotoLine(0,0); cout<<hpmax<<":"<<atk<<":"<<def;
}
void Player::displayZoom(int ****map, int length_x, int length_y){
		...
		for(int j=0;j<vision*2+1;j++){
			if(pow(i-vision,2)+pow(j-vision,2)<=pow(vision,2)){
				if(read.y<0 || read.x<0 || read.y>length_y-1 || read.x>length_x-1){
					...
					cout<<"EB"; //Monster
					zoom[i][j]=5;
				}else if(map[read.y][read.x][read.b][read.a]==6){
					cout<<"HK"; //Equipment
					zoom[i][j]=6;
...
void Player::walk(int ****map, vector<Equipment> &eq){
	if(delay_move==0){
		Pos copy=pos;
